...
![Rockpool Detective](https://i.imgur.com/LkocSwY.png)
...
### Player choice
This was made using a click event, and corresponded with the values in the `choiceArray`. I used the `forEach` method to add the event listener. The `playerChoice` function added player selected choices into the `playerCurrentChoiceArray`. Once the length of the array was equal to four, no further choices were added.
...
#### Winning and losing
The `computerCurrentChoiceArray` and the `playerCurrentChoiceArray` are compared to see if they are a direct match. If the player wins, the gameover screen class is added to the display, and the winning text is shown
...
Secondly I checked to see if the value in the `computerCurrentChoiceArray` value matches any value in the `playerCurrentChoiceArray`. The partial matches would be given a value of `partial` in both objects.  If the key already had a value (`full`) no value would be added to the object, preventing duplication.
...
To display the number and colour of clue shells required, any match values (`full`, `partial`) are passed into the `clueArray` at the end of each round. This is displayed in the DOM as either white ( value = `partial`) or black (value = `full`)shells. 
...
* Utilising vanilla JavaScript for DOM manipulation .
